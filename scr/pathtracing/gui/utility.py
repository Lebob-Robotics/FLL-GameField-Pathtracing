...
class FileUtility:
    ...
    def import_image(*path, alpha = True):
        fullPath = os.path.join(FileUtility.absolute_path, *path)
    ...
    def import_folder(*path) -> list[pygame.Surface]:
        ...
        for folderPath, _, imageNames in os.walk(os.path.join(FileUtility.absolute_path, *path)):
    ...
    def import_folder_dict(*path) -> dict[str, pygame.Surface]:
        ...
        for folderPath, _, image_names in os.walk(os.path.join(FileUtility.absolute_path, *path)):
            image_names = [name for name in image_names if name.endswith( ".png")]
            for image_name in [name.split('.')[0] for name in image_names]:
    ...
    def import_subfolders(*path) -> dict[str, list[pygame.Surface]]:
        ...
        for _, subfolders, _ in os.walk(os.path.join(FileUtility.absolute_path, *path)):
